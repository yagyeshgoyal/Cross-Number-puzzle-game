...
const ResultDisplay = ({ isCorrect, wrongRules, onNextPuzzle, isLastPuzzle, showNextButton }) => {
  if (isCorrect) {
    return (
      <div className="bg-white rounded-lg shadow-xl p-6 mb-6">
        <div className="p-6 bg-green-100 border-2 border-green-500 rounded-lg text-center">
          ...
          {isLastPuzzle && (
            <div className="mt-4">
              <p className="text-2xl font-bold text-purple-700">🏆 You completed all puzzles!</p>
            </div>
          )}
        </div>
      </div>
          ...
          {wrongRules.map((wrong, idx) => (
            <div key={wrong.id} className="bg-white p-5 rounded-lg border-2 border-red-300">
              ...
              <div className="bg-green-50 p-4 rounded-lg border-2 border-green-400">
                ...
                <p className="text-xs text-green-600 mt-2">💡 Make sure your numbers follow this rule exactly</p>
              </div>
            </div>
          ))}
        </div>
